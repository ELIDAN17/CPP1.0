...
void ArbolElectoral::insertar(Votante v)
{
    ...
    {
        y = x;
        if (v.dni < x->votante.dni)
            x = x->izquierdo;
        else if (v.dni > x->votante.dni)
            x = x->derecho;
        else
        {
            x->votante = v;
    ...
    if (y == NIL)
        raiz = z;
    else if (z->votante.dni < y->votante.dni)
        y->izquierdo = z;
    else
        y->derecho = z;
    fixInsertar(z);
...
bool ArbolElectoral::p4(NodoRBT *n) const
{
    ...
    {
        if (n->izquierdo->color == Color::ROJO || n->derecho->color == Color::ROJO)
            return false;
    }
