...
if modulo != "Asignación de Disco":
...
else: # MÓDULO DE ASIGNACIÓN DE DISCO
    st.subheader("📁 Simulación Dinámica de Asignación de Espacio en Disco")
    disco = st.session_state.disco
    
    # Formulario para Crear Archivo en el panel principal
    c1, c2, c3 = st.columns([1.5, 1, 1.5])
    with c1:
        nuevo_nombre = st.text_input("Nombre del archivo (ej. tesis.pdf):", value="documento.txt")
    with c2:
        nuevo_tam = st.number_input("Tamaño (en bloques):", min_value=1, max_value=20, value=3)
        # Mostrar equivalencia en KB en tiempo real abajo del input
        st.caption(f"Equivale a: **{nuevo_tam * 4} KB** de datos")
    with c3:
        metodo_asig = st.selectbox("Método de Asignación:", ["Contigua", "Enlazada", "Indexada"])
    
    col_btn1, col_btn2 = st.columns(2)
    with col_btn1:
        if st.button("📥 Crear y Guardar Archivo", use_container_width=True):
            if nuevo_nombre in disco.archivos:
                st.error(f"El archivo '{nuevo_nombre}' ya existe en el disco.")
            else:
                if metodo_asig == "Contigua":
                    exito, msg = disco.asignar_contigua(nuevo_nombre, nuevo_tam)
                elif metodo_asig == "Enlazada":
                    exito, msg = disco.asignar_enlazada(nuevo_nombre, nuevo_tam)
                else:
                    exito, msg = disco.asignar_indexada(nuevo_nombre, nuevo_tam)
                ...
                if exito:
                    ...
                    st.error(msg)
    with col_btn2:
        if st.button("🧹 Formatear Disco (Limpiar Todo)", use_container_width=True):
            st.session_state.disco = ld.DiscoSimulado(limite_bloques)
            st.rerun()

    st.markdown("---")

    # Grid de Visualización de Bloques de Disco
    st.write("### 🎛️ Mapa Físico del Disco Duro")
    st.caption("Nota: Cada bloque equivale a **4 KB** de almacenamiento físico.")
    ...
    for i in range(disco.total_bloques):
        ...
        color_bloque = "#e2e8f0"  # Gris por defecto (vacío)
        texto_b = f"B{i}<br><span style='font-size:9px; opacity:0.6;'>(4K)</span>"
        borde = "1px solid #cbd5e1"
        
        if cont:
            if cont.startswith("ÍNDICE_"):
                archivo_asoc = cont.replace("ÍNDICE_", "")
                color_bloque = "#f59e0b"  # Color ámbar para índices
                texto_b = f"<b>Índice</b><br><span style='font-size:9px;'>{archivo_asoc}</span>"
        ...
        # AQUÍ ESTÁ LA CORRECCIÓN CLAVE: El HTML se concatena en una sola línea limpia sin sangría de Python
        html_disco += f'<div style="background-color: {color_bloque}; border: {borde}; border-radius: 6px; padding: 10px 5px; text-align: center; font-size: 11px; font-weight: bold; color: {"white" if cont and not cont.startswith("ÍNDICE_") else "#1e293b"}; min-height: 55px; display: flex; flex-direction: column; justify-content: center; align-items: center;">{texto_b}</div>'
        
    html_disco += '</div>'
    st.markdown(html_disco, unsafe_allow_html=True)
    
    # Pestañas informativas y de eliminación de archivos
    tab1, tab2 = st.columns([2.5, 1.5])
    with tab1:
        st.write("### 📋 Tabla de Contenido del Disco (Directorio)")
        if not disco.archivos:
            st.info("El disco está vacío. Añade algunos archivos para ver su estructura lógica.")
        else:
            filas = []
            for nom, info in disco.archivos.items():
                idx_info = disco.bloques_indices.get(nom, "No tiene")
                total_blq = len(info["bloques"]) + (1 if idx_info != "No tiene" else 0)
                filas.append({
                    "Nombre de Archivo": nom,
                    "Método": info["tipo"],
                    "Bloques Ocupados": str(info["bloques"]),
                    "Bloque Índice": idx_info,
                    "Total Bloques": total_blq,
                    "Tamaño Real": f"{total_blq * 4} KB" # Traducimos los bloques a tamaño legible
                })
            st.dataframe(pd.DataFrame(filas), use_container_width=True)
    with tab2:
        st.write("### 🗑️ Eliminar Archivo")
        if disco.archivos:
            archivo_a_eliminar = st.selectbox("Seleccione el archivo a remover:", list(disco.archivos.keys()))
            if st.button("❌ Liberar Espacio", use_container_width=True):
                if disco.eliminar_archivo(archivo_a_eliminar):
                    st.success(f"Archivo '{archivo_a_eliminar}' eliminado exitosamente.")
                    st.rerun()
                else:
                    st.error("Error al intentar eliminar el archivo.")
        else:
            st.caption("No hay archivos cargados que se puedan eliminar.")